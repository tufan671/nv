...
mongoose.connect('mongodb+srv://Arcenio:mea3jq14v1@cluster0.qyyew.mongodb.net/myFirstDatabase?retryWrites=true&w=majority', {useNewUrlParser: true, useUnifiedTopology: true});// Mongo connect linki
// Recep Efsanesi 
    ...
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
      if (!inviterData) {
        ...
        newInviter.save().then(x => {
          ...
          Toplam Davet Etmis Üye ${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)} Token Ve Yeni Acilen Hesapları İnvite Olarak Saymıyoruz <:guard1:922196249384476732>`).catch(err => {});
              });
      } else {
        inviterData.fake++
    ...
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
        if (!inviterData) {
          ...
          newInviter.save().then(x => {
            ...
            Toplam Davet Etmis Üye ${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)} Güvenilir Hesap <a:andreo_giveaway:922195061339459645>)`).catch(err => {});
          });
        } else {
          inviterData.regular++;
          inviterData.save().then(x => {
